...
- arcUserId: string — stable messenger identity (from `ARC_USER_ID`) for multi-account isolation.
...
{
  source: "matrix",
  arcUserId: "messenger@ach9:endurance.network",
  eventId: "$NJibtC...",
...
Events:
- Unique event: `{ source: 1, arcUserId: 1, eventId: 1 }` with `partialFilterExpression: { eventId: { $type: 'string' } }` (Matrix receipts may lack eventId).
- Room timeline: `{ arcUserId: 1, roomId: 1, timestamp: 1, _id: 1 }` (range scans and stable ordering).
- Sender timeline: `{ arcUserId: 1, senderId: 1, timestamp: 1 }`.
- Relations: `{ arcUserId: 1, "relatesTo.eventId": 1, timestamp: 1 }`.
- Type-time: `{ arcUserId: 1, type: 1, timestamp: 1 }` (sampling/ops).
- Receipt de-duplication (no eventId): unique index on `{ source: 1, arcUserId: 1, type: 1, roomId: 1, senderId: 1, "relatesTo.eventId": 1 }` with partial filter `{ type: 'receipt' }`.

Projections:
- `message_state`: `{ arcUserId: 1, roomId: 1, messageId: 1 }` unique.
- `reaction_index`: `{ arcUserId: 1, messageId: 1, key: 1 }` unique.
- `read_index`: `{ arcUserId: 1, messageId: 1, userId: 1 }` unique.
...
- Uses existing `ARC_DATABASE_URI` and `DB_NAME` (`arcRecursiveCore`).
...
- Sharding (future): shard by `{ arcUserId, roomId }` or `{ arcUserId, timestamp }` depending on query patterns.
...
- Receipt de-dup complexity: use compound unique index on `{ source, arcUserId, type, roomId, senderId, relatesTo.eventId }`.
...
- Room timeline page:
  - Query `events` by `{ arcUserId, roomId }` sorted by `timestamp`, `limit/skip`.

- Message with state:
  - Fetch base message from `events` where `{ type:'message', eventId }`.
  - Join with `message_state` by `{ arcUserId, messageId: eventId }`.

- Reactions for a message:
  - Query `reaction_index` by `{ arcUserId, messageId }` to get counts and user sets.

- Read status:
  - Query `read_index` by `{ arcUserId, messageId }` for reader list and latest ts.
...
const EventsSchema = new Schema(
  {
    ...
    arcUserId: { type: String, index: true },
...
EventsSchema.index({ arcUserId: 1, roomId: 1, timestamp: 1, _id: 1 });
EventsSchema.index({ arcUserId: 1, senderId: 1, timestamp: 1 });
EventsSchema.index({ arcUserId: 1, 'relatesTo.eventId': 1, timestamp: 1 });
EventsSchema.index({ arcUserId: 1, type: 1, timestamp: 1 });
EventsSchema.index(
