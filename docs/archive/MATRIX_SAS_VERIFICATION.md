...
- Optional import:
  - Set `MATRIX_USER_RECOVERY_KEY_B64` to a 32-byte base64 key to allow SDK-led import of cross-signing / backup secrets.
  - The app provides `cryptoCallbacks.getSecretStorageKey` when valid.

Example env var:
```
MATRIX_USER_RECOVERY_KEY_B64=base64_of_32_bytes_key
```
